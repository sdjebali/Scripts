...
# Provides:
###########
# - a tsv file with header that has gene pairs that are on the same chr and closer than 4Mb
#   with their two gene ids first, then their tss positions, then their tss to tss distance
#   (<=4Mb) and finally the union of their enhancers that are between 25kb and 2Mb from their
#   tss and the intersection of their enhancers that are between 25kb and 2Mb from their tss
#   as two numbers and finally the corresponding jaccard percentage
...
# time $pgm $genepairs $genetsscoord $contactedenhancers > genepairs2commonenhancers.out 2> genepairs2commonenhancers.err
...
then
    ...
    echo "  then the cardinal of the intersection of those and then the Jaccard percentage out of those two" >&2 
...
awk -v fileRef=$tsscoord 'BEGIN{OFS="\t"; while (getline < fileRef >0){tsschr[$1]=$3; tsspos[$1]=$4}} NR==1{print "gnid1", "gnid2", "tsspos1", "tsspos2", "tssdist"} NR>=2{c1=tsschr[$1]; c2=tsschr[$2]; p1=tsspos[$1]; p2=tsspos[$2]; d=(c1==c2 ? abs(p1-p2) : "NA"); if(c1==c2&&d<=4000000){print $1, $2, c1":"p1, c2":"p2, d}} function abs(x){return (x>=0 ? x : (-1*x))}' $gnpair > realparalogs.closer4Mb.gn1.gn2.tss1.tss2.dist.tsv
echo done >&2

# 2. Add the list of non redundant enhancers of gene1 that are at a [25kb; 2Mb] distance from the tss of gene1 and the same for gene2
######################################################################################################################################
echo "I am adding the list of non redundant enhancers of gene1 that are between 25kb and 2Mb from its tss and the same for gene2" >&2
awk -v fileRef1=$tsscoord -v fileRef2=$contactenh -f $addenh realparalogs.closer4Mb.gn1.gn2.tss1.tss2.dist.tsv > realparalogs.closer4Mb.gn1.gn2.tss1.tss2.dist.nrenhokdist.gn1.gn2.tsv 
echo done >&2

# 3. Add the nr enh list made of the union of both lists and then the intersection list, and the lengths of those two
#####################################################################################################################
#    as well as the jaccard percentage made by dividing the 2nd by the 1st and multiplying by 100
#################################################################################################
echo "I am adding the " >&2
awk -f $unioninter realparalogs.closer4Mb.gn1.gn2.tss1.tss2.dist.nrenhokdist.gn1.gn2.tsv > realparalogs.closer4Mb.gn1.gn2.tss1.tss2.dist.nrenhokdist.gn1.gn2.union.inter.lists.tsv
echo done >&2


# 4. Make a file with the same info but replacing the two last lists by their lengths and the jaccard percentage
################################################################################################################
#    made by dividing the 2nd by the 1st and multiplying by 100
###############################################################
echo "" >&2
awk 'NR==1{OFS="\t"; print $1, $2, $3, $4, $5, "nunion", "ninter", "jaccard.pcent"}  {nu=split($8,a,","); ni=split($9,b,","); print $1, $2, $3, $4, $5, nu-1, ni-1, (ni-1)/(nu-1)*100}' realparalogs.closer4Mb.gn1.gn2.tss1.tss2.dist.nrenhokdist.gn1.gn2.union.inter.lists.tsv > realparalogs.closer4Mb.gn1.gn2.tss1.tss2.dist.contactedenhancers.union.inter.nb.jaccardpcent.tsv
echo done >&2


# 5. Computes the distribution of the jaccard percentage
########################################################
echo "I am computing the distribution of the jaccard percentage" >&2
awk 'NR>=2{print $NF}' realparalogs.closer4Mb.gn1.gn2.tss1.tss2.dist.contactedenhancers.union.inter.nb.jaccardpcent.tsv > tmp
$stats tmp
rm tmp
echo done >&2
