...
# Given a tsv file with header that has gene pairs separated by a dist provided in column 5 (otherwise NA if diff chr or too distant)
# and in the last two columns the nr comma separated list of enhancers contacted by each gene at a reasonable distance (25kb-2Mb)
# on the same chr for the gene pairs separated by less than 4Mb, will add the list of nr enhancers in the union and in the intersection
...
# main output file = realparalogs.gn1.gn2.tss1.tss2.dist.nrenhokdist.gn1.gn2.union.inter.tsv
...
NR>=2{
	...
	# we go over the enh list of gene1 and for the current gene pair (row here) we store whether each enhancer that is not NA was seen for gene1 in seen1
